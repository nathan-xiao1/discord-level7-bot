...
// Entry functions
module.exports = {
  kick: function kick(args, message) {
    if (!args.length || !message.mentions.users.size) {
      return message.channel.send(`⚠ **Usage**: \`${prefix}kick @user\``);
    }
  ...
  unkick: function unkick(args, message) {
    if (!args.length || !message.mentions.users.size) {
      return message.channel.send(`⚠ **Usage**: \`${prefix}unkick @user\``);
    }
    message.mentions.members.forEach((member) => {
      unvote(message, member);
    });
  },

  viewKicks: function viewKicks(args, message) {
    view(message);
  },
};

// Functions
async function vote(message, member) {
  ...
  if (voters.includes(authorID)) {
    return message.channel.send(
      `**⚠ You have already voted for <@${userID}>**`
    );
  }
...
async function unvote(message, member) {
  const userID = member.user.id;
  const guildMembers = member.guild.memberCount;
  const authorID = message.author.id;
  ...
  if (!entry) {
    return message.channel.send(
      `**⚠ There are current no vote for <@${userID}>**`
    );
  }
...
async function view(message) {
  ...
  const embed = new Discord.MessageEmbed().setTitle(`**Vote Kick Records**`);
